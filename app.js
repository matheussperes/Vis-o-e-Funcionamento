...
const ROTAS = [
  ...
  { padrao: /^#\/preferencias$/, tela: telaPreferencias, nav: '#/preferencias' },
...
async function telaAreas() {
  ...
  atualizarNavAreas();   /* Bloco 5.1: sidebar acompanha as mudanças */
}
...
async function telaMetas() {
  atualizarNavMeta();          /* Bloco 6: card da sidebar acompanha as mudanças */
...
/* ── Bloco 5 — radar SVG de equilíbrio por área (sem bibliotecas) ──
   eixos: [{ nome, cor, valor }] · valores normalizados pelo maior. */
function svgRadarAreas(eixos) {
  const W = 300, H = 232, cx = W / 2, cy = H / 2 + 4, R = 76;
  const n = eixos.length;
  const maxV = Math.max(...eixos.map((e) => e.valor), 1);
  const ponto = (i, r) => {
    const ang = -Math.PI / 2 + (i * 2 * Math.PI) / n;
    return [cx + r * Math.cos(ang), cy + r * Math.sin(ang)];
  };
  const anel = (f) => eixos.map((_, i) => ponto(i, R * f).map((v) => v.toFixed(1)).join(',')).join(' ');
  const dados = eixos.map((e, i) => ponto(i, R * (e.valor / maxV)).map((v) => v.toFixed(1)).join(',')).join(' ');

  const rotulos = eixos.map((e, i) => {
    const [x, y] = ponto(i, R + 14);
    const anc = Math.abs(x - cx) < 8 ? 'middle' : (x > cx ? 'start' : 'end');
    const nome = e.nome.length > 14 ? e.nome.slice(0, 13) + '…' : e.nome;
    return `<text x="${x.toFixed(1)}" y="${(y + 3).toFixed(1)}" text-anchor="${anc}"
      font-size="10" fill="${e.cor}">${esc(nome)}</text>`;
  }).join('');

  return `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Radar de equilíbrio por área"
      style="width:100%;max-width:340px;display:block;margin:0 auto;">
    ${[0.33, 0.66, 1].map((f) =>
      `<polygon points="${anel(f)}" fill="none" stroke="var(--borda)" stroke-width="1"/>`).join('')}
    ${eixos.map((_, i) => {
      const [x, y] = ponto(i, R);
      return `<line x1="${cx}" y1="${cy}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}" stroke="var(--borda)" stroke-width="1"/>`;
    }).join('')}
    <polygon points="${dados}" fill="rgba(var(--foco-rgb),0.28)" stroke="var(--foco)" stroke-width="2" stroke-linejoin="round"/>
    ${eixos.map((e, i) => {
      const [x, y] = ponto(i, R * (e.valor / maxV));
      return `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="3" fill="${e.cor}"/>`;
    }).join('')}
    ${rotulos}
  </svg>`;
}
...
function desenharVisao(ctx) {
  ...
  /* ── Bloco 5 (fase B) — Heatmap de carga semanal ──
     Σ minutos alocados por dia × bloco da semana corrente (todas as
     tarefas, abertas e concluídas: a foto da semana não míngua
     conforme você conclui). Intensidade relativa ao pico. */
  const diasSemana = Array.from({ length: 7 }, (_, i) => addDiasISO(ctx.segunda, i));
  const cargaCel = new Map();
  const cargaDia = new Map();
  ctx.alocsSemana.forEach((al) => {
    const t = mapT.get(al.tarefa_id);
    if (!t) return;
    const min = t.estimativa_min || 0;
    const k = al.data + '|' + al.bloco;
    cargaCel.set(k, (cargaCel.get(k) || 0) + min);
    cargaDia.set(al.data, (cargaDia.get(al.data) || 0) + min);
  });
  const picoCel = Math.max(0, ...cargaCel.values());
  const htmlHeatmap = picoCel === 0
    ? '<p class="meta-texto">Nada alocado nesta semana ainda. <button class="btn btn-fantasma btn-mini" data-ir="#/ritual">Planejar →</button></p>'
    : `<div class="heat-grid">
        <span></span>
        ${diasSemana.map((d, i) => `
          <span class="heat-dia${d === ctx.hoje ? ' heat-hoje' : ''}">
            <strong>${DIAS_SEMANA_LABEL[i]}</strong><br>
            <span class="meta-texto">${fmtData(d).slice(0, 5)}</span><br>
            <span class="meta-texto">${cargaDia.get(d) ? fmtMin(cargaDia.get(d)) : '—'}</span>
          </span>`).join('')}
        ${['manha', 'tarde', 'noite'].map((b) => `
          <span class="heat-rotulo meta-texto">${BLOCO_LABEL[b]}</span>
          ${diasSemana.map((d) => {
            const v = cargaCel.get(d + '|' + b) || 0;
            const alfa = v === 0 ? 0 : 0.14 + 0.56 * (v / picoCel);
            return `<span class="heat-cel${d === ctx.hoje ? ' heat-hoje' : ''}"
              style="${v ? `background:rgba(var(--foco-rgb),${alfa.toFixed(2)});` : ''}"
              title="${BLOCO_LABEL[b]} de ${fmtData(d)}: ${v ? fmtMin(v) : 'livre'}">${v ? fmtMin(v) : ''}</span>`;
          }).join('')}`).join('')}
      </div>
      <p style="margin-top:8px;"><button class="btn btn-fantasma btn-mini" data-ir="#/semana">abrir Semana →</button></p>`;

  /* ── Bloco 5 (fase B) — Radar de equilíbrio por área ──
     Base: minutos planejados nesta semana por área (mesma régua do
     KPI); semana vazia → cai para o trabalho aberto por área. */
  const areasAtivas = ctx.areas.filter((a) => !a.arquivada);
  const minPorArea = new Map();
  let baseRadar = 'planejado nesta semana';
  idsPlanejadasSemana.forEach((id) => {
    const t = mapT.get(id);
    const aid = t && t._proj ? t._proj.area_id : null;
    if (aid) minPorArea.set(aid, (minPorArea.get(aid) || 0) + (t.estimativa_min || 0));
  });
  if (minPorArea.size === 0) {
    baseRadar = 'trabalho aberto';
    ctx.tarefas.forEach((t) => {
      const aid = t._proj ? t._proj.area_id : null;
      if (aid) minPorArea.set(aid, (minPorArea.get(aid) || 0) + (t.estimativa_min || 0));
    });
  }
  const eixos = areasAtivas.map((a) => ({ nome: a.nome, cor: a.cor, valor: minPorArea.get(a.id) || 0 }));
  const totalRadar = eixos.reduce((s, e) => s + e.valor, 0);
  const htmlRadar = (eixos.length < 3 || totalRadar === 0)
    ? `<p class="meta-texto">${eixos.length < 3
        ? 'O radar precisa de ao menos 3 áreas ativas.'
        : 'Sem tempo registrado por área ainda — crie tarefas com estimativa.'}</p>`
    : svgRadarAreas(eixos) +
      `<p class="meta-texto" style="margin-top:6px;">Base: ${baseRadar} · ${fmtMin(totalRadar)} no total</p>
       <ul class="lista-simples radar-legenda">${eixos
         .filter((e) => e.valor > 0)
         .sort((a, b) => b.valor - a.valor)
         .map((e) => `<li class="vg-linha"><span class="dot" style="background:${e.cor}"></span>
           <span class="titulo-trunc">${esc(e.nome)}</span>
           <span class="meta-texto">${fmtMin(e.valor)} · ${Math.round((e.valor / totalRadar) * 100)}%</span></li>`).join('')}</ul>`;

  /* ── Bloco 5.1 — Linha do tempo de projetos ──
     Janela: 2 semanas atrás → 6 semanas à frente. Barra = do cadastro
     (created_at) ao prazo, na cor da área, com o progresso preenchido
     por dentro e % na ponta. Sem prazo = barra tracejada até o fim da
     janela. Marcador vertical "Hoje". Clique na linha abre o projeto. */
  const diasEntre = (a, b) =>
    Math.round((new Date(b + 'T12:00:00Z') - new Date(a + 'T12:00:00Z')) / 86400000);
  const tlIni = addDiasISO(ctx.hoje, -14);
  const tlFim = addDiasISO(ctx.hoje, 42);
  const tlSpan = diasEntre(tlIni, tlFim);
  const tlPos = (iso) => Math.min(100, Math.max(0, (diasEntre(tlIni, iso) / tlSpan) * 100));

  let tlTicks = '';
  for (let d = segundaDaSemana(tlIni); d <= tlFim; d = addDiasISO(d, 7)) {
    if (d < tlIni) continue;
    tlTicks += `<span class="tl-tick" style="left:${tlPos(d).toFixed(2)}%">${fmtData(d).slice(0, 5)}</span>`;
  }

  const tlLinhas = cartoesProjetos.slice(0, 10).map(({ p, area, pctP, dias }) => {
    const cor = area ? area.cor : '#8FA3B8';
    const ini = (() => { const c = dataSP(p.created_at); return c > tlIni ? c : tlIni; })();
    const fimReal = p.prazo || tlFim;
    const fim = fimReal < tlIni ? tlIni : (fimReal > tlFim ? tlFim : fimReal);
    const e = tlPos(ini);
    const dPct = Math.max(tlPos(fim) - e, 2);
    const vencido = dias !== null && dias < 0;
    return `<button class="tl-linha" data-ir="#/projeto/${p.id}" title="${esc(p.nome)} · ${pctP}%${p.prazo ? ` · até ${fmtData(p.prazo)}` : ' · sem prazo'}">
      <span class="tl-nome"><span class="dot" style="background:${cor}"></span><span class="titulo-trunc">${esc(p.nome)}</span></span>
      <span class="tl-trilha">
        <span class="tl-hoje-l" style="left:${tlPos(ctx.hoje).toFixed(2)}%"></span>
        <span class="tl-bar${p.prazo ? '' : ' tl-sem-prazo'}" style="left:${e.toFixed(2)}%;width:${dPct.toFixed(2)}%;background:${cor}40;border-color:${cor}66;">
          <span class="tl-prog" style="width:${pctP}%;background:${cor};"></span>
        </span>
        <span class="tl-pct${vencido ? ' vg-alerta' : ''}" style="left:${Math.min(tlPos(fim) + 1, 92).toFixed(2)}%">${pctP}%${vencido ? ' ⚠' : ''}</span>
      </span>
    </button>`;
  }).join('');

  const htmlTimeline = cartoesProjetos.length === 0
    ? '<p class="meta-texto">Nenhum projeto ativo para mostrar na linha do tempo.</p>'
    : `<div class="tl">
        <div class="tl-cab">
          <span class="tl-nome"></span>
          <span class="tl-trilha tl-trilha-cab">${tlTicks}
            <span class="tl-hoje-rotulo" style="left:${tlPos(ctx.hoje).toFixed(2)}%">Hoje</span>
          </span>
        </div>
        ${tlLinhas}
      </div>
      ${cartoesProjetos.length > 10 ? `<p class="meta-texto">+ ${cartoesProjetos.length - 10} projeto(s) — janela de 8 semanas</p>` : ''}`;
  ...
  elView.innerHTML = `
    ...
    <div class="visao-grid">
      ...
      <section class="widget widget-largo"><h2>Carga da semana</h2>${htmlHeatmap}</section>
      <section class="widget"><h2>Equilíbrio por área</h2>${htmlRadar}</section>
      <section class="widget widget-todo"><h2>Linha do tempo</h2>${htmlTimeline}</section>
...
/* ════════════════════════════════════════════════════════════════
   BLOCO 6 — PREFERÊNCIAS DO PAINEL (painel de administrador)
   Liga/desliga abas da sidebar desktop + card da Meta principal +
   escolha de qual meta aparece no card. Guardado em localStorage
   (por aparelho). Mobile mantém a navegação completa. Preferências
   e Sair nunca podem ser ocultadas (anti-travamento).
   ════════════════════════════════════════════════════════════════ */

const CHAVE_PREFS = 'vf_prefs';
const PREFS_ABAS = [
  ['visao', 'Visão Geral'],
  ['agora', 'Agora'],
  ['hoje', 'Hoje'],
  ...
  ['navAreas', 'Bloco "Áreas da vida"'],
  ['metaCard', 'Card "Meta principal"']
];

function lerPrefs() {
  let salvas = {};
  try { salvas = JSON.parse(localStorage.getItem(CHAVE_PREFS)) || {}; } catch (e) { salvas = {}; }
  const abas = {};
  PREFS_ABAS.forEach(([k]) => { abas[k] = (salvas.abas && salvas.abas[k] === false) ? false : true; });
  return { abas, metaPrincipal: salvas.metaPrincipal || 'auto' };
}

function salvarPrefs(prefs) {
  try { localStorage.setItem(CHAVE_PREFS, JSON.stringify(prefs)); } catch (e) { /* sem storage, sem persistência */ }
  aplicarPrefs();
}

/* Esconde/mostra os elementos marcados com data-pref (só no desktop, via CSS) */
function aplicarPrefs() {
  const p = lerPrefs();
  $$('[data-pref]').forEach((el) =>
    el.classList.toggle('pref-oculta', p.abas[el.dataset.pref] === false));
}

/* Card da Meta principal no rodapé da sidebar. Progresso = tarefas
   concluídas/total dos projetos vinculados à meta. Meta exibida:
   a escolhida nas Preferências, ou a ativa de prazo mais próximo. */
async function atualizarNavMeta() {
  const wrap = $('#nav-meta');
  if (!wrap || !estado.sessao) return;
  const prefs = lerPrefs();
  if (!prefs.abas.metaCard) return;          /* já oculto via aplicarPrefs */
  try {
    const [metas, projetos, tarefas] = await Promise.all([
      qMetas(), qProjetos(),
      obter(sb.from('tarefas').select('projeto_id, status'))
    ]);
    const ativas = metas.filter((m) => m.status === 'ativa');
    let m = prefs.metaPrincipal !== 'auto'
      ? ativas.find((x) => x.id === prefs.metaPrincipal) || null : null;
    if (!m) {
      m = [...ativas].sort((a, b) => {
        if (!a.prazo && !b.prazo) return a.created_at < b.created_at ? -1 : 1;
        if (!a.prazo) return 1;
        if (!b.prazo) return -1;
        return a.prazo < b.prazo ? -1 : 1;
      })[0] || null;
    ...
    const idsProj = new Set(projetos.filter((p) => p.meta_id === m.id).map((p) => p.id));
    const doM = tarefas.filter((t) => idsProj.has(t.projeto_id));
    const total = doM.length;
    const feitas = doM.filter((t) => t.status === 'concluida').length;
    const pct = total ? Math.round((feitas / total) * 100) : 0;
    const dias = m.prazo
      ? Math.round((new Date(m.prazo + 'T12:00:00Z') - new Date(hojeISO() + 'T12:00:00Z')) / 86400000)
      : null;

    wrap.innerHTML = `
      <p class="meta-texto nav-meta-rotulo">Meta principal</p>
      <p class="nav-meta-titulo">${esc(m.titulo)}</p>
      ${m.resultado_esperado ? `<p class="meta-texto titulo-trunc">${esc(m.resultado_esperado)}</p>` : ''}
      ${m.prazo ? `<p class="meta-texto">até ${fmtData(m.prazo)} · <span class="${dias < 0 ? 'vg-alerta' : ''}">${dias < 0 ? 'vencida' : dias + 'd'}</span></p>` : ''}
      <span class="barra-prog"><span class="barra-prog-cheia" style="width:${pct}%;background:var(--foco);"></span></span>
      <p class="meta-texto">${total ? `${feitas}/${total} tarefas · ${pct}%` : 'sem projetos vinculados'}</p>
      <button class="btn btn-fantasma btn-mini" data-meta-det>Ver detalhes →</button>`;
    $('[data-meta-det]', wrap).addEventListener('click', () => { location.hash = '#/metas'; });
    wrap.classList.remove('hidden');
  } catch (e) { /* card é conveniência */ }
}

/* Tela Preferências (#/preferencias) — o painel de administrador */
async function telaPreferencias() {
  elView.innerHTML = `<h1>Preferências</h1><div class="skeleton skeleton-card"></div>`;
  let metas = [];
  try { metas = (await qMetas()).filter((m) => m.status === 'ativa'); } catch (e) { metas = []; }
  const prefs = lerPrefs();

  elView.innerHTML = `
    <h1>Preferências</h1>
    <p class="meta-texto">Vale para este aparelho. No celular a navegação permanece completa; "Preferências" e "Sair" ficam sempre visíveis.</p>

    <div class="card" style="margin-bottom:16px;">
      <h2 style="font-size:15px; margin-bottom:8px;">Abas da sidebar (desktop)</h2>
      ${PREFS_ABAS.map(([k, rotulo]) => `
        <label class="pref-linha">
          <input type="checkbox" data-pref-aba="${k}" ${prefs.abas[k] ? 'checked' : ''}>
          <span>${rotulo}</span>
        </label>`).join('')}
    </div>

    <div class="card">
      <h2 style="font-size:15px; margin-bottom:8px;">Meta principal (card da sidebar)</h2>
      <div class="campo">
        <select data-pref-meta>
          <option value="auto"${prefs.metaPrincipal === 'auto' ? ' selected' : ''}>Automática — meta ativa de prazo mais próximo</option>
          ${metas.map((m) => `<option value="${m.id}"${prefs.metaPrincipal === m.id ? ' selected' : ''}>${esc(m.titulo)}${m.prazo ? ` (até ${fmtData(m.prazo)})` : ''}</option>`).join('')}
        </select>
      </div>
      ${metas.length === 0 ? '<p class="meta-texto">Nenhuma meta ativa — crie uma em Metas para o card aparecer.</p>' : ''}
    </div>`;

  $$('[data-pref-aba]').forEach((cb) => cb.addEventListener('change', () => {
    const p = lerPrefs();
    p.abas[cb.dataset.prefAba] = cb.checked;
    salvarPrefs(p);
    if (cb.dataset.prefAba === 'metaCard' && cb.checked) atualizarNavMeta();
    toast(cb.checked ? 'Aba visível ✓' : 'Aba oculta ✓');
  }));

  $('[data-pref-meta]').addEventListener('change', (ev) => {
    const p = lerPrefs();
    p.metaPrincipal = ev.target.value;
    salvarPrefs(p);
    atualizarNavMeta();
...
/* Bloco 5.1: bloco "Áreas da vida" na sidebar (desktop). Decorativo e
   navegacional — clique leva ao Panorama. Atualizado no login e quando
   a tela Áreas altera algo. */
async function atualizarNavAreas() {
  const wrap = $('#nav-areas');
  if (!wrap || !estado.sessao) return;
  try {
    const areas = await qAreas();
    wrap.innerHTML = areas.filter((a) => !a.arquivada).map((a) => `
      <button class="nav-area" data-rota-area>
        <span class="dot" style="background:${a.cor}"></span>
        <span class="titulo-trunc">${esc(a.nome)}</span>
      </button>`).join('');
    $$('[data-rota-area]', wrap).forEach((b) =>
      b.addEventListener('click', () => { location.hash = '#/panorama'; }));
  } catch (e) { /* sidebar é conveniência */ }
}
...
async function iniciar() {
  ...
  sb.auth.onAuthStateChange((_evento, sessao) => {
    ...
    if (!tinha && sessao) { atualizarNavAreas(); atualizarNavMeta(); }   /* Blocos 5.1/6 */
    if (sessao && filaReenvio.length > 0) reenviarFila();   /* seção 16: reexecuta pós-login */
  });

  aplicarPrefs();                                            /* Bloco 6: abas conforme o painel */
  if (estado.sessao) { atualizarNavAreas(); atualizarNavMeta(); }   /* sessão já restaurada */
