...
const ROTAS = [
  ...
  { padrao: /^#\/templates$/, tela: telaTemplates, nav: null },
...
async function destinoInicial() {
  try {
    ...
    return projetos.length === 0 ? '#/wizard' : '#/agora';
  } catch (e) {
    return '#/agora';
  }
}
...
async function consultarCopiloto(raiz, ctx) {
  ...
  try {
    /* Passos pendentes das 10 primeiras da fila + compras pendentes */
    const top = ctx.fila.slice(0, 10);
    let subs = [], compras = [];
    ...
    out.innerHTML = `
      <div class="cp-acao card card-elevado">
        <p class="meta-texto">${esc(data.acao.projeto || '')}</p>
        <h3 style="margin:4px 0 8px;">${esc(data.acao.titulo)}</h3>
        <p>${esc(data.acao.motivo || '')}</p>
        ${data.acao.primeiro_passo ? `<p style="margin-top:8px;"><strong>Comece por:</strong> ${esc(data.acao.primeiro_passo)}</p>` : ''}
        ${data.acao.tempo ? `<p class="meta-texto" style="margin-top:8px;">⏱ ${esc(data.acao.tempo)}</p>` : ''}
      </div>
      ${data.plano_b ? `<p class="meta-texto" style="margin-top:8px;"><strong>Plano B</strong> (${esc(data.plano_b.quando || 'se travar')}): ${esc(data.plano_b.titulo)}${data.plano_b.projeto ? ` · ${esc(data.plano_b.projeto)}` : ''}</p>` : ''}
      ${(data.alertas || []).length ? `<div class="cp-alertas">${data.alertas.map((a) =>
        `<p class="meta-texto">⚠ <strong>${esc(a.projeto)}</strong> — ${esc(a.descricao)}</p>`).join('')}</div>` : ''}
      ${data.observacao ? `<p class="meta-texto" style="margin-top:8px;">${esc(data.observacao)}</p>` : ''}`;
  } catch (e) {
